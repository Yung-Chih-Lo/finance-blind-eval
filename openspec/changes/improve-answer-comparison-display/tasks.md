...
- [x] 6.7 W1b (layout, found in visual review): a markdown **table** still clipped card text. Root cause — `.answer-card` is `display: grid` with an implicit `auto` column that the table's intrinsic width expanded past the card; `min-width: 0` on the grid item alone did not help because the *track*, not the item, was growing. Fix in `web/app/globals.css`: add `grid-template-columns: minmax(0, 1fr)` to `.answer-card` (an `fr` track is sized by free space only, never content max-content) and keep `min-width: 0` on `.answer-body`. Broaden the "Rendered media does not blow out a card" spec scenario to cover wide content such as tables. Verified by build + visual confirmation in the participant card preview.
