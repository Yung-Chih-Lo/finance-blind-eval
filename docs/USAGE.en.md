...
| `provider` | Editable API base URL, optional models endpoint override, model picker (calls derived `${base}/models` by default, overridable via `OPENAI_COMPAT_MODELS_ENDPOINT`), system prompt, user-prompt template, temperature, max tokens; includes a "test call" preview that does NOT write to records |
